...
// HISTORY:
// 7.0m - (03.05.2014) Исходный файл от AVSel с форума фонарёвки
// 7.1m - (17.04.2020) Переработанная программа
// 7.2m - (06.05.2024) Переработан и вынесен в отдельный файл анализ состояния нажатий кнопок.
//                     Индикация уровня аккумулятора перенесена на тройной клик.
//                     Переименование некоторых переменных.
// 7.3m - (06.05.2024) Включение коротким нажатием на той же яркости, на которой выключили.
//                     После включения длительным удержанием ждём отпускания кнопки (не даём увеличивает яркость).
// 7.4m - (12.05.2024) После выхода из режима маячка ждём отпускания кнопки.
// 7.5m - (16.05.2024) Для отладки добавлена возможность вывода напряжения аккумулятора и 
//                     температуры по UART (PB0 pin, 7874 bits/s). Вывод по 3 click + удержание.
//                     (UART включается определением символа UART_DEBUG в настройках компилятора)
...
//                     - GetTemp() возвращает температуру в Цельсиях. В T_OFFSET добавлена коррекция на основе реальных измерений
//                     - TempOff в EEPROM сохраняется в Цельсиях
//                     - TempOff по умолчанию уменьшен с 75°С до 65°С
//===========================================================================
